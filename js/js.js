...
document.getElementById(
  "numerodepaises"
).innerHTML = `Actualmente <span>${numeroDePaisesQueRequierenPermiso()}</span> nacionalidades lo requieren`;
...
paisSeleccionado.addEventListener("change", (evento) => {
  ...
  const ultimoPaisVisitado = {
    pais: buscarId.pais,
    bandera: buscarId.emoji,
    permiso: buscarId.permiso,
  };
...
console.log("Si estás viendo esto, sabes más de Javascript que nosotros.");
// Uso de SPREAD y Math.max:
const precioResidencia = paises.map(function (elprecio) {
  return `${elprecio.costoResidencia}`;
});
console.log(
  "El costo del permiso de residencia más caro asciende a $" +
    Math.max(...precioResidencia) +
    " dólares de los Estados Unidos."
);
console.log("Los países disponibles y su bandera se muestran a continuación:");
// Incorporando Desestructuración:
